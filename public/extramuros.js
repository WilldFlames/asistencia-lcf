...
function extEncabezadoAnexo(numero,titulo,e,a){return `<div style="position:relative;text-align:center;margin-bottom:8px;"><div style="position:absolute;right:0;top:0;border:1.5px solid #1f3864;border-radius:5px;padding:5px 8px;font-size:9px;font-weight:bold;color:#1f3864;">${htmlSeguro(extConsecutivo(e.consecutivo,a.anio))}</div><div style="font-size:9px;">Circular DM-00013-05-2019</div><div style="font-weight:bold;font-size:12px;margin-top:2px;">ANEXO ${numero}</div><div style="font-weight:bold;font-size:12px;text-transform:uppercase;margin-top:2px;">${htmlSeguro(titulo)}</div></div>`;}
...
function extTablaEstilo(){return "width:100%;border-collapse:collapse;font-size:9.3px;line-height:1.22";}
function extTd(){return "border:1px solid #555;padding:4px;vertical-align:top";}

function extPaginaAnexo1(d,e){const a=d.actividad,resp=d.responsables.map(extNombrePersona).join(", ");return `<div style="font-family:Arial,sans-serif;font-size:9.8px;line-height:1.28;color:#111;">${extEncabezadoAnexo(1,"Autorización para la participación de la persona estudiante",e,a)}<p style="text-align:justify;margin:5px 0;">Las personas docentes responsables <strong>${htmlSeguro(resp)}</strong> solicitan autorización para que la persona estudiante que se indica participe en la actividad educativa fuera de la institución.</p>${extLinea("Nombre de la persona estudiante",extNombrePersona(e))}${extLinea("Nivel que cursa",`${extNivelTexto(e.nivel)} (${e.seccion_nombre||""})`)}${extLinea("Nombre de la persona encargada",extNombreEncargado(e))}${extLinea("Cédula de la persona encargada",e.encargado_cedula||"")}<table style="${extTablaEstilo()};margin-top:7px;"><tr><td style="${extTd()};width:31%"><strong>Nombre de la actividad</strong></td><td style="${extTd()}">${htmlSeguro(a.nombre_actividad)}</td></tr><tr><td style="${extTd()}"><strong>Lugar</strong></td><td style="${extTd()}">${htmlSeguro(a.lugar_actividad)}</td></tr><tr><td style="${extTd()}"><strong>Fecha y horario</strong></td><td style="${extTd()}">${htmlSeguro(extFechaLarga(a.fecha_actividad))}, salida ${htmlSeguro(extHora(a.hora_salida))}, regreso ${htmlSeguro(extHora(a.hora_regreso))}</td></tr><tr><td style="${extTd()}"><strong>Objeto</strong></td><td style="${extTd()}">${htmlSeguro(a.objeto_actividad)}</td></tr></table><p style="margin:7px 0 4px;"><strong>Autorización:</strong></p><p style="margin:3px 0;">☐ Sí autorizo &nbsp;&nbsp;&nbsp;&nbsp; ☐ No autorizo la participación de la persona estudiante.</p><table style="${extTablaEstilo()};margin-top:7px;"><tr><td style="${extTd()};width:34%"><strong>Condición médica o enfermedad</strong></td><td style="${extTd()}">${htmlSeguro(e.enfermedad||"No registrada")}</td></tr><tr><td style="${extTd()}"><strong>Medicamento e indicaciones</strong></td><td style="${extTd()}">${htmlSeguro(e.medicamento||"No registrado")}</td></tr><tr><td style="${extTd()}"><strong>Teléfonos de emergencia</strong></td><td style="${extTd()}">${htmlSeguro(extTelefonos(e)||"No registrados")}</td></tr><tr><td style="${extTd()}"><strong>Correo</strong></td><td style="${extTd()}">${htmlSeguro(e.encargado_email||"No registrado")}</td></tr></table><div style="display:grid;grid-template-columns:1fr 1fr;gap:28px;margin-top:28px;text-align:center;"><div style="border-top:1px solid #333;padding-top:4px;">Firma de la persona encargada</div><div style="border-top:1px solid #333;padding-top:4px;">Fecha</div></div></div>`;}

function extPaginaAnexo2(d,e){const a=d.actividad,filas=d.responsables.map((p,i)=>`<tr><td style="${extTd()}">${i+1}</td><td style="${extTd()}">${htmlSeguro(extNombrePersona(p))}</td><td style="${extTd()}">&nbsp;</td><td style="${extTd()}">&nbsp;</td></tr>`).join("");const firmas=d.responsables.map(p=>`<div style="border-top:1px solid #333;padding-top:3px;text-align:center;">${htmlSeguro(extNombrePersona(p))}<br/>Persona responsable</div>`).join("");return `<div style="font-family:Arial,sans-serif;font-size:9.2px;line-height:1.2;color:#111;">${extEncabezadoAnexo(2,"Plan de la actividad fuera del centro educativo",e,a)}<div style="font-size:9px;margin-bottom:5px;"><strong>Persona estudiante:</strong> ${htmlSeguro(extNombrePersona(e))} &nbsp; · &nbsp; <strong>Sección:</strong> ${htmlSeguro(e.seccion_nombre||"—")}</div><table style="${extTablaEstilo()}"><tr><td style="${extTd()};width:29%"><strong>Nombre de la actividad</strong></td><td style="${extTd()}">${htmlSeguro(a.nombre_actividad)}</td></tr><tr><td style="${extTd()}"><strong>Objeto de la actividad</strong></td><td style="${extTd()}">${htmlSeguro(a.objeto_actividad)}</td></tr><tr><td style="${extTd()}"><strong>Actividades y mediación pedagógica</strong></td><td style="${extTd()}">${htmlSeguro(a.mediacion_pedagogica)}</td></tr><tr><td style="${extTd()}"><strong>Lugar de ejecución</strong></td><td style="${extTd()}">${htmlSeguro(a.lugar_actividad)}</td></tr><tr><td style="${extTd()}"><strong>Fecha y horario</strong></td><td style="${extTd()}">${htmlSeguro(extFechaLarga(a.fecha_actividad))}, de ${htmlSeguro(extHora(a.hora_salida))} a ${htmlSeguro(extHora(a.hora_regreso))}</td></tr></table><h4 style="margin:7px 0 3px;">Personas funcionarias responsables</h4><table style="${extTablaEstilo()}"><thead><tr><th style="${extTd()};width:5%">#</th><th style="${extTd()}">Nombre completo</th><th style="${extTd()};width:22%">Teléfono</th><th style="${extTd()};width:22%">Firma</th></tr></thead><tbody>${filas}</tbody></table><h4 style="margin:7px 0 3px;">Documentación y previsiones</h4><table style="${extTablaEstilo()}"><tr><td style="${extTd()}">☐ Autorización de la persona encargada</td><td style="${extTd()}">☐ Exoneración de responsabilidad</td></tr><tr><td style="${extTd()}">☐ Lista de participantes</td><td style="${extTd()}">☐ Medidas de seguridad previstas</td></tr></table><div style="margin-top:7px;"><strong>Observaciones:</strong><div style="border:1px solid #777;min-height:38px;padding:5px;margin-top:3px;white-space:pre-wrap;">${htmlSeguro(a.observaciones||"")}</div></div><p style="margin:7px 0;">Se presenta este plan para conocimiento y autorización de la Dirección del centro educativo.</p><div style="display:grid;grid-template-columns:repeat(${Math.min(3,Math.max(1,d.responsables.length))},1fr);gap:18px;margin-top:20px;">${firmas}</div><div style="width:260px;margin:25px auto 0;border-top:1px solid #333;padding-top:3px;text-align:center;"><strong>${htmlSeguro(d.config.director_nombre||"Dirección")}</strong><br/>Persona directora · Visto bueno</div></div>`;}

function extPaginaAnexo3(d,e){const a=d.actividad,enc=extNombreEncargado(e);return `<div style="font-family:Arial,sans-serif;font-size:10.2px;line-height:1.45;color:#111;">${extEncabezadoAnexo(3,"Exoneración de responsabilidad por accidente",e,a)}<p style="text-align:justify;margin-top:16px;">Yo, <strong>${htmlSeguro(enc||"________________________________")}</strong>, portador(a) de la cédula de identidad número <strong>${htmlSeguro(e.encargado_cedula||"________________")}</strong>, en calidad de ${htmlSeguro(e.encargado_parentesco||"persona encargada legal")} de la persona estudiante <strong>${htmlSeguro(extNombrePersona(e))}</strong>, de la sección <strong>${htmlSeguro(e.seccion_nombre||"—")}</strong>, manifiesto que conozco y autorizo ${htmlSeguro(a.descripcion_exoneracion)}.</p><p style="text-align:justify;">Declaro que he recibido información sobre el objeto, lugar, fecha, horario y personas responsables de la actividad. Asimismo, comunicaré oportunamente cualquier condición médica o indicación especial que deba considerarse para proteger la salud y seguridad de la persona estudiante.</p><p style="text-align:justify;">En consecuencia, y salvo que medie dolo o culpa grave comprobada, exonero de responsabilidad al Ministerio de Educación Pública, al Liceo de Calle Fallas y a las personas funcionarias responsables por accidentes fortuitos que puedan ocurrir durante el traslado o desarrollo normal de la actividad.</p><table style="${extTablaEstilo()};margin:20px 0;"><tr><td style="${extTd()};width:31%"><strong>Actividad</strong></td><td style="${extTd()}">${htmlSeguro(a.nombre_actividad)}</td></tr><tr><td style="${extTd()}"><strong>Lugar</strong></td><td style="${extTd()}">${htmlSeguro(a.lugar_actividad)}</td></tr><tr><td style="${extTd()}"><strong>Fecha y horario</strong></td><td style="${extTd()}">${htmlSeguro(extFechaLarga(a.fecha_actividad))}, de ${htmlSeguro(extHora(a.hora_salida))} a ${htmlSeguro(extHora(a.hora_regreso))}</td></tr></table><p>Firmo en Desamparados, a los ______ días del mes de __________________ del año ${htmlSeguro(a.anio)}.</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:34px;margin-top:55px;text-align:center;"><div style="border-top:1px solid #333;padding-top:5px;">Firma de la persona encargada<br/>${htmlSeguro(enc)}</div><div style="border-top:1px solid #333;padding-top:5px;">Número de cédula<br/>${htmlSeguro(e.encargado_cedula||"")}</div></div></div>`;}
