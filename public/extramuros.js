...
function extPaginaAnexo1(d,e){
  ...
  return `<div style="font-family:Arial,sans-serif;font-size:9px;line-height:1.22;color:#111;">${extEncabezadoAnexo(1,"Autorización de participación de la población estudiantil",e,a)}
  ...
  <div style="border:1px solid #555;border-top:0;padding:5px 7px;text-align:justify;"><div style="font-weight:bold;text-align:center;margin-bottom:3px;">Autorización de participación</div>Mediante el presente documento, yo <strong>${extDato(enc)}</strong>, cédula de identidad <strong>${extDato(e.encargado_cedula)}</strong>, en calidad de <strong>${extDato(e.encargado_parentesco)}</strong>, autorizo la participación de la persona estudiante <strong>${extDato(extNombrePersona(e))}</strong> en la actividad denominada <strong>${extDato(a.nombre_actividad)}</strong>, actividad que se ejecutará fuera del centro educativo en <strong>${extDato(a.lugar_actividad)}</strong>, el día <strong>${extDato(extFechaLarga(a.fecha_actividad))}</strong>.</div>
  <div style="border:1px solid #555;border-top:0;padding:5px 7px;text-align:justify;"><div style="font-weight:bold;margin-bottom:3px;">Consentimiento informado para el uso de fotografías, videos y otros con fines educativos</div>Autorizo a su vez el uso de la imagen y el nombre del estudiante, que sean recopilados mediante fotografías, videos, textos y otros por parte del personal docente y autoridades del Ministerio de Educación Pública. Esta autorización se brinda únicamente con fines educativos, para la promoción y divulgación de actividades organizadas por el centro educativo o el Ministerio de Educación Pública.<div style="margin-top:4px;">☐ SÍ &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ☐ NO</div></div>
  <div style="border:1px solid #555;border-top:0;padding:5px 7px;min-height:44px;"><strong>Información médica para considerar por el centro educativo:</strong><div style="margin-top:4px;">${extInfoMedica(e)||"&nbsp;<br/>&nbsp;"}</div></div>
  <div style="border:1px solid #555;border-top:0;padding:5px 7px;text-align:justify;"><strong>Observaciones:</strong> Únicamente los estudiantes que presenten la información completa solicitada en el Anexo 2, junto con las copias solicitadas en el Anexo 2 y las firmas correspondientes en los anexos 1 y 3, podrán participar de la actividad.</div>
  <table style="${extTablaEstilo()}"><tr><td style="${extTd()};width:31%;text-align:center;"><strong>Información de contacto del encargado legal</strong></td><td style="${extTd()}"><strong>Teléfonos:</strong> ${extDato(extTelefonos(e))}<br/><strong>Correo electrónico:</strong> ${extDato(e.encargado_email)}</td></tr><tr><td style="${extTd()};text-align:center;"><strong>Fecha de firma</strong></td><td style="${extTd()};height:22px;">&nbsp;</td></tr><tr><td style="${extTd()};text-align:center;"><strong>Firma del encargado legal</strong></td><td style="${extTd()};height:28px;">&nbsp;</td></tr></table></div>`;
}
...
function extPaginaAnexo2(d,e){
  ...
  return `<div style="font-family:Arial,sans-serif;font-size:8.7px;line-height:1.16;color:#111;">${extEncabezadoAnexo(2,"Formulario · Plan de actividad ejecutada fuera del centro educativo",e,a)}
  <div style="font-size:8.5px;margin-bottom:3px;"><strong>Persona estudiante:</strong> ${extDato(extNombrePersona(e))} · <strong>Sección:</strong> ${extDato(e.seccion_nombre)}</div>
...
function extPaginaAnexo3(d,e){
  ...
  return `<div style="font-family:Arial,sans-serif;font-size:9.25px;line-height:1.36;color:#111;">${extEncabezadoAnexo(3,"Exoneración de responsabilidad de salida del estudiante del centro educativo en caso de accidente",e,a)}
  <p style="text-align:justify;margin:12px 0 8px;">Yo, <strong>${extDato(enc)}</strong>, con identificación N.° <strong>${extDato(e.encargado_cedula)}</strong>, responsable legal del estudiante <strong>${extDato(extNombrePersona(e))}</strong>, de la sección <strong>${extDato(e.seccion_nombre)}</strong>; de acuerdo con lo que establece el artículo 5 del Código de la Niñez y la Adolescencia y el Código de Familia, en el ejercicio pleno de la patria potestad que me confiere la ley; exonero a los funcionarios del Liceo de Calle Fallas de toda responsabilidad legal en casos de fuerza mayor o caso fortuito, culpa de la víctima o hechos de un tercero, según lo establece la Ley General de la Administración Pública, artículo 190, inciso 1, que se pueda presentar en el desarrollo de <strong>${extDato(a.descripcion_exoneracion)}</strong>.</p>
  <p style="text-align:justify;margin:8px 0;">Asimismo, ratifico que asumo la responsabilidad de cualquier accidente o lesión que pueda tener mi hijo(a) y que se pueda derivar del trayecto de ida y regreso. También exonero de toda responsabilidad a los docentes que acompañan como representantes de la institución para esta actividad, así como a la persona directora del Liceo de Calle Fallas, por cualquier accidente, lesión, enfermedad, muerte o problema que pueda suceder en la salida, traslado y durante el tiempo de permanencia del estudiante durante la actividad.</p>
  <p style="text-align:justify;margin:8px 0;">Si me retraso en recoger a mi hijo(a) a la hora correspondiente indicada en la boleta de permiso de salida de Giras Educativas, exonero al Centro Educativo de toda responsabilidad en caso de que se pueda presentar alguna situación que pueda lesionar su integridad y estoy en el entendido de que se aplicará el procedimiento de llamar al PANI y a la Policía de Proximidad para la debida protección de la persona menor de edad.</p>
  <p style="text-align:justify;margin:8px 0;">Declaro que, como encargado legal, apoyaré la organización y desarrollo de la actividad; notificaré a los responsables algún padecimiento de salud que pueda presentar mi hijo(a), el cual debe ser del conocimiento del Centro Educativo; y llenaré debidamente la boleta de datos personales del estudiante sin omitir información importante para salvaguardar la atención médica, si así lo amerita, durante el desarrollo de la actividad.</p>
  <p style="margin:12px 0 0;">Firmo en la ciudad de Desamparados, a los ______ días del mes de __________________ del año <strong>${htmlSeguro(a.anio)}</strong>.</p>
  <div style="display:grid;grid-template-columns:1.5fr .9fr;gap:48px;margin-top:52px;text-align:center;"><div style="border-top:1px solid #333;padding-top:5px;">Nombre y firma del encargado legal<br/><strong>${extDato(enc)}</strong></div><div style="border-top:1px solid #333;padding-top:5px;">Número de identificación<br/><strong>${extDato(e.encargado_cedula)}</strong></div></div></div>`;
}
