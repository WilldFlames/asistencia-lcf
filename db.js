...
async function initDB() {
  ...
  try {
    ...
    // ── ALERTA TEMPRANA MEP ────────────────────────────────────────────────
    // La alerta pertenece al docente/asignación que la abre. El historial de
    // seguimientos nunca se sobrescribe: cada cambio queda como una fila nueva.
    await client.query(`
      CREATE TABLE IF NOT EXISTS alertas_tempranas (
        ...
        seccion_id           INTEGER REFERENCES secciones(id) ON DELETE SET NULL,
        estado               TEXT NOT NULL DEFAULT 'activada'
                             CHECK(estado IN ('activada','en_proceso','en_espera','cerrada','eliminada')),
        riesgo_ausentismo    BOOLEAN NOT NULL DEFAULT false,
        categorias           JSONB NOT NULL DEFAULT '[]'::jsonb,
        observacion_inicial  TEXT DEFAULT '',
        fecha_activacion     DATE NOT NULL DEFAULT CURRENT_DATE,
        fecha_cierre         DATE,
        created_at           TIMESTAMP DEFAULT NOW(),
        updated_at           TIMESTAMP DEFAULT NOW()
      );
      CREATE TABLE IF NOT EXISTS alerta_temprana_seguimientos (
        id             SERIAL PRIMARY KEY,
        alerta_id      INTEGER NOT NULL REFERENCES alertas_tempranas(id) ON DELETE CASCADE,
        estado         TEXT NOT NULL CHECK(estado IN ('activada','en_proceso','en_espera','cerrada','eliminada')),
        observaciones  TEXT NOT NULL,
        registrado_por INTEGER NOT NULL REFERENCES usuarios(id) ON DELETE RESTRICT,
        fecha          DATE NOT NULL DEFAULT CURRENT_DATE,
        created_at     TIMESTAMP DEFAULT NOW()
      );
      CREATE TABLE IF NOT EXISTS alerta_temprana_acciones (
        id                    SERIAL PRIMARY KEY,
        alerta_id             INTEGER NOT NULL REFERENCES alertas_tempranas(id) ON DELETE CASCADE,
        accion                TEXT NOT NULL,
        fecha_inicio          DATE,
        ...
        observaciones         TEXT DEFAULT '',
        institucion_referida  TEXT DEFAULT '',
        registrado_por       INTEGER NOT NULL REFERENCES usuarios(id) ON DELETE RESTRICT,
        created_at            TIMESTAMP DEFAULT NOW(),
        updated_at            TIMESTAMP DEFAULT NOW()
      );
      CREATE TABLE IF NOT EXISTS registro_llamadas (
        ...
        asignacion_id          INTEGER REFERENCES asignaciones(id) ON DELETE SET NULL,
        materia_id             INTEGER REFERENCES materias(id) ON DELETE SET NULL,
        alerta_id              INTEGER REFERENCES alertas_tempranas(id) ON DELETE SET NULL,
        fecha                  DATE NOT NULL DEFAULT CURRENT_DATE,
        hora_inicio            TIME NOT NULL,
        hora_fin               TIME,
        medio                  TEXT NOT NULL CHECK(medio IN ('telefono','videollamada','audio','otro')),
        medio_otro             TEXT DEFAULT '',
        numero_marcado         TEXT DEFAULT '',
        resultado              TEXT NOT NULL CHECK(resultado IN ('efectiva','no_contesta','equivocado','fuera_servicio','buzon','devolver_llamada','otro')),
        resultado_otro         TEXT DEFAULT '',
        atendio_nombre         TEXT DEFAULT '',
        parentesco             TEXT DEFAULT '',
        parentesco_otro        TEXT DEFAULT '',
        motivos                JSONB NOT NULL DEFAULT '[]'::jsonb,
        motivo_otro            TEXT DEFAULT '',
        descripcion_situacion  TEXT DEFAULT '',
        respuesta_encargado    TEXT DEFAULT '',
        compromisos_encargado  TEXT DEFAULT '',
        compromisos_docente    TEXT DEFAULT '',
        fecha_seguimiento      DATE,
        observaciones          TEXT DEFAULT '',
        created_at             TIMESTAMP DEFAULT NOW()
      );
      CREATE TABLE IF NOT EXISTS alerta_temprana_contactos (
        id                 SERIAL PRIMARY KEY,
        alerta_id          INTEGER NOT NULL REFERENCES alertas_tempranas(id) ON DELETE CASCADE,
        llamada_id         INTEGER UNIQUE REFERENCES registro_llamadas(id) ON DELETE CASCADE,
        fecha              DATE NOT NULL DEFAULT CURRENT_DATE,
        via_contacto       TEXT NOT NULL,
        persona_contactada TEXT DEFAULT '',
        comentarios        TEXT DEFAULT '',
        registrado_por     INTEGER NOT NULL REFERENCES usuarios(id) ON DELETE RESTRICT,
        created_at         TIMESTAMP DEFAULT NOW()
      );
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_at_estudiante ON alertas_tempranas(estudiante_id,anio,estado)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_at_profesor ON alertas_tempranas(profesor_id,anio,updated_at DESC)`);
    ...
    // Permite que la campana abra directamente la pantalla y el registro.
    await client.query(`ALTER TABLE notificaciones ADD COLUMN IF NOT EXISTS destino TEXT DEFAULT NULL`);
